...
namespace PiggE {
    ...
    {
        PIG_CORE_ERROR("GLFW Error ({0} : {1})", error, description);
    }
    ...
    void WindowsWindow::Init(const WindowProps& props)
    {
        ...
        PIG_CORE_INFO("Creating window {0} ({1}, {2})", props.Title, props.Width, props.Height);

        if (!s_GLFWInitialized)
        {
            // TODO: glfwTerminate on system shutdown
            int success = glfwInit();
            PIG_CORE_ASSERT(success, "Could not intialize GLFW!");
